...
case "$5" in 
	...
	"hg38")
		echo "not build";exit 1;;
	*)	echo "genome reference is required"; exit 1;;
esac
...
#filter 
awk -f $SCRIPT/pipe_pickup.awk $UG_OUTFILE/$UG_NAME.snp.filter.tmp >$UG_OUTFILE/$UG_NAME.snp.filter
awk -f $SCRIPT/pipe_pickup.awk $UG_OUTFILE/$UG_NAME.indel.filter.tmp >$UG_OUTFILE/$UG_NAME.indel.filter
awk -f $SCRIPT/pipe_pickup.awk $HC_OUTFILE/$HC_NAME.snp.filter.tmp >$HC_OUTFILE/$HC_NAME.snp.filter
awk -f $SCRIPT/pipe_pickup.awk $HC_OUTFILE/$HC_NAME.indel.filter.tmp >$HC_OUTFILE/$HC_NAME.indel.filter

#in both methods
perl $SCRIPT/pipe_inBothUG_HC.pl $UG_OUTFILE/$UG_NAME.snp.filter $HC_OUTFILE/$HC_NAME.snp.filter $GATK_OUTFILE/$1.$2.snp.final.vcf

perl $SCRIPT/pipe_inBothUG_HC.pl $UG_OUTFILE/$UG_NAME.indel.filter $HC_OUTFILE/$HC_NAME.indel.filter $GATK_OUTFILE/$1.$2.indel.final.vcf
